...
def compare(x, y):
    ...
    x_j, y_j = greedy_transform(x), greedy_transform(y)
    fns = [is_five_of_kind, is_four_of_kind, is_full_house, is_three_of_kind, is_two_pair, is_one_pair]
    x_apply, y_apply = [fn(x_j) for fn in fns], [fn(y_j) for fn in fns]
    for i, (xx, yy) in enumerate(zip(x_apply, y_apply)):
        if xx and not yy:
            return -1
        if yy and not xx:
            return 1
...
def greedy_transform(hand):
    ...
    candidates = sorted(candidates, key=cmp_to_key(first_compare), reverse=True)
    return ''.join([candidates[0] if c == 'J' else c for c in hand])
