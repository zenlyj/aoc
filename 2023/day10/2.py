...
def is_contained(grid, pos, loop):
    num_exit = 0
    i = pos[0]
    for j in range(pos[1]):
        if (i, j) not in loop:
            continue
        # hard-coded S to be NORTH facing pipe
        # TODO replace S with actual pipe programmatically
        if grid[i][j] in {'|', 'J', 'L', 'S'}:
            num_exit += 1
    return num_exit % 2 != 0
    ...
    for d in DIRECTIONS:
        ...
        while curr_pos and curr_direction and not is_out_of_bounds(curr_pos) and grid[curr_pos[0]][curr_pos[1]] != '.':
            if curr_pos == start_point:
                loop = visited if len(visited) > len(loop) else loop
                break
